    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="animate-pulse flex flex-col items-center gap-3">
          ...
          <p className="text-slate-500 text-sm">{t("timeline.loading")}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="sticky top-0 z-40 bg-primary text-white">
        <div className="max-w-lg mx-auto md:max-w-4xl px-4 h-14 flex items-center gap-3">
          ...
          <h1 className="text-base font-bold">{t("timeline.title")}</h1>
          <span className="ml-auto text-xs bg-white/20 px-2 py-0.5 rounded-full">
            {t("timeline.eventsCount", { count: timeline.length })}
          </span>
        </div>
      </header>
      ...
      <main className="max-w-lg mx-auto md:max-w-4xl px-4 py-4 space-y-4">
        ...
        {(staleCards.length > 0 || outdatedCards.length > 0) && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex items-start gap-3">
            ...
            <div className="flex-1 text-sm text-amber-900">
              <p className="font-medium">{t("timeline.dataFreshnessWarning")}</p>
              <p className="text-amber-700 mt-0.5">
                {staleCards.length > 0 && (
                  <span>
                    {t("timeline.profilesStale", { count: staleCards.length })}
                  </span>
                )}
                {staleCards.length > 0 && outdatedCards.length > 0 && <span>, </span>}
                {outdatedCards.length > 0 && (
                  <span>
                    {t("timeline.profilesOutdated", { count: outdatedCards.length })}
                  </span>
                )}
              </p>
            </div>
        ...
        {timeline.length === 0 ? (
          <div className="bg-white rounded-lg border border-slate-200 p-8 text-center">
            ...
            <p className="text-slate-500 text-sm">{t("timeline.noEvents")}</p>
          </div>
        ) : (
          <div className="relative">
              ...
              {timeline.map((entry, idx) => {
                    ...
                    <div key={`alert-${alert.id}`} className="relative ps-12 md:ps-14">
                      ...
                      <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 space-y-1">
                        ...
                        <p className="text-xs text-amber-600">
                          {getZoneName(alert.zone, locale)} &middot; {t("timeline.actorsInvolved", { count: alert.actorCount })}
                        </p>
                      </div>
                    </div>
                  );
                ...
                return (
                  <div key={`change-${change.id}`} className="relative ps-12 md:ps-14">
                    {/* Timeline dot */}
                    <div className="absolute start-3.5 top-4 w-3 h-3 rounded-full bg-primary ring-2 ring-blue-100 md:left-4.5" />
                    ...
                    <div className="bg-white border border-slate-200 rounded-lg p-4 space-y-2">
                      ...
                      <div className="flex items-center gap-2 text-sm bg-slate-50 rounded-xl px-3 py-2">
                        ...
                        <span className="font-semibold text-primary">{change.newValue}</span>
                      </div>
                    </div>
                  </div>
                );
